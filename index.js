...
const octokit = new Octokit({
  ...
  // GitHub repo details
  const owner = 'Testyy12';
  const repo = 'apotu';
  const path = 'auth.json';
  
  async function validatePairingNumber(lilychan, phoneNumber) {
    try {
      ...
      const allowedNumbers = JSON.parse(content);
  
      // Check if number is already allowed
      if (allowedNumbers.includes(phoneNumber)) {
        console.log(chalk.green('Number already authorized'));
        return true;
      }
  
      // If not allowed, notify admin
      const adminNumber = '6285785313072@s.whatsapp.net';
      
      await lilychan.sendMessage(adminNumber, {
        text: `*Request Akses Bot*\n\nNomor: ${phoneNumber}\nWaktu: ${moment().tz('Asia/Jakarta').format('DD/MM/YY HH:mm:ss')}\n\nBalas dengan:\n.berikan-akses (untuk memberikan akses)\n.gak (untuk menolak)`,
      });
  
      // Wait for admin response
      return new Promise((resolve) => {
        lilychan.ev.once('messages.upsert', async (chatUpdate) => {
          try {
            ...
            if (from === adminNumber) {
              if (body === '.berikan-akses') {
                // Add number to allowed list
                allowedNumbers.push(phoneNumber);
                ...
                await octokit.repos.createOrUpdateFileContents({
                  ...
                  repo,
                  path,
                  message: `Add new allowed number: ${phoneNumber}`,
                  content: Buffer.from(JSON.stringify(allowedNumbers, null, 2)).toString('base64'),
                  sha: fileData.sha
                });
  
                // Notify success
                await lilychan.sendMessage(adminNumber, {
                  text: `✅ Akses diberikan untuk nomor ${phoneNumber}`
                });
  
                console.log(chalk.green(`Access granted for number: ${phoneNumber}`));
                resolve(true);
  
              } else if (body === '.gak') {
                // Notify rejection
                await lilychan.sendMessage(adminNumber, {
                  text: `❌ Akses ditolak untuk nomor ${phoneNumber}`
                });
  
                console.log(chalk.red('Access denied. Stopping bot...'));
                process.exit(1);
              }
            }
          } catch (err) {
        ...
        setTimeout(() => {
          console.log(chalk.yellow('No response received from admin. Stopping bot...'));
          process.exit(1);
        }, 300000);
      });
    } catch (error) {
      console.error('Error in validatePairingNumber:', error);
      return false;
    }
  }
...
⠨⡂⡀⢑⢕⡅⠂⠄⠉⠛⠻⠿⢿⣿⣿⣿⣿⣿⣿⣿⣿⡿⢋⢔⢕⢕⣿⣿⠠⠈
⠄⠪⣂⠁⢕⠆⠄⠂⠄⠁⡀⠂⡀⠄⢈⠉⢍⢛⢛⢛⢋⢔⢕⢕⢕⣽⣿⣿⠠⠈`);
console.log(chalk.cyan('Validating number access...'));
const isValidated = await validatePairingNumber(lilychan, global.pairing);

if (!isValidated) {
    console.log(chalk.red('Number not validated. Stopping bot...'));
    process.exit(1);
}

console.log(chalk.green('Number validated. Proceeding with pairing...'));
...
//jangan di hapus ntar erro
try {
    await lilychan.newsletterFollow('')
} catch (error) {
    
 console.log('abaikan saya')
